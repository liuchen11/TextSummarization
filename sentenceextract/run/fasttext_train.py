...
assert(fasttext_model_params['sequence_length']==my_data_generator.sentence_length_threshold)
assert(fasttext_model_params['vocab_size']==my_data_generator.word_list_length+2)
assert(fasttext_model_params['embedding_dim']==my_embedding_loader.embedding_dim)
...
for batch_idx in xrange(begin_batch_idx,batch_num+begin_batch_idx):
    input_matrix, masks, labels, _, extension_part=my_data_generator.batch_gen(set_label='train',batch_size=my_network.batch_size,
        extend_tags=[],label_policy='min',model_tag='fasttext')
    ...
    if (batch_idx+1)%validation_frequency==0:   # Validation Checkpoint
        ...
        for validation_idx in xrange(validation_batches):
            input_matrix, masks, labels, _, extension_part=my_data_generator.batch_gen(set_label='validate',batch_size=my_network.batch_size,
                extend_tags=[],label_policy='min',model_tag='fasttext')
