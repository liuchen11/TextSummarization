...
if len(sys.argv)!=2:
    print('Usage: python run_naive_extractor.py <config>')
    exit(0)
...
fasttext_params=network_params['fasttext_params']
model2load=network_params['model2load']

assert(fasttext_params['sequence_length']==my_data_generator.sentence_length_threshold)
assert(fasttext_params['vocab_size']==my_data_generator.word_list_length+2)
assert(fasttext_params['embedding_dim']==my_embedding_loader.embedding_dim)
if 'pretrain_embedding' in network_params and network_params['pretrain_embedding']==True:
    fasttext_params['embedding_matrix']=embedding_matrix

my_network=fasttext.fastText(fasttext_params)
...
for idx, file_name in enumerate(file_list):
    ...
    while True:
        input_matrix,masks,labels,stop,extend_part=my_data_generator.batch_gen(set_label=file_name,batch_size=1,
            label_policy='min', extend_tags=[], model_tag='fasttext')
        result_this_document['ground_truth'].append(labels[0])
        important_idx=my_network.test(inputs=input_matrix, masks=masks, fine_tune=True)[0][1]
        result_this_document['result'].append((len(result_this_document['result']), important_idx))
        if stop:
            break
    ...
    if sys.version_info.major==2:
        ...
        result_this_document['result']=sorted(result_this_document['result'], key=lambda x:x[1], reverse=True)
