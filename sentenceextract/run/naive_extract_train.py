...
if len(sys.argv)!=2:
    print('Usage: python naive_extract_train.py <config>')
    exit(0)
...
for batch_idx in xrange(begin_batch_idx,batch_num+begin_batch_idx):
    input_matrix,masks,labels,_,extension_part=my_data_generator.batch_gen(set_label='train',batch_size=my_network.batch_size,
        extend_tags=input_extend_tags,label_policy='min',model_tag='naive')
    _, loss=my_network.train(input_matrix,labels,extension_part)
    ...
    if (batch_idx+1)%validation_frequency==0:       # start validation
        ...
        for validation_batch_idx in xrange(validation_batches):
            input_matrix,masks,labels,_,extension_part=my_data_generator.batch_gen(set_label='validate',batch_size=my_network.batch_size,
                extend_tags=input_extend_tags,label_policy='min',model_tag='naive')
            _, loss=my_network.validate(input_matrix,labels,extension_part)
