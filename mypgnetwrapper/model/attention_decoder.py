...
def attention_decoder(decoder_inputs, initial_state, encoder_states, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None):
  ...
  Args:
    ...
    encoder_states: 3D Tensor [batch_size x attn_length x attn_size].     attn_length = sequence length, attn_size = hidden dimension
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    encoder_features = nn_ops.conv2d(encoder_states, W_h, [1, 1, 1, 1], "SAME") # shape (batch_size,attn_length,1,attention_vec_size)   # Equivalent with fully connected layer, mapping all hidden states to embeddings.
    ...
    def attention(decoder_state, coverage=None):
      ...
      with variable_scope.variable_scope("Attention"):
        ...
        if use_coverage and coverage is not None: # non-first step of coverage
          ...
          e = math_ops.reduce_sum(v * math_ops.tanh(encoder_features + decoder_features + coverage_features), [2, 3])  # shape (batch_size,attn_length) ## BROADCAST
          ...
          if use_coverage: # first step of training
            coverage = tf.expand_dims(tf.expand_dims(attn_dist,2),2) # initialize coverage  # of shape [batch_size, atten_length,1,1]

        # Calculate the context vector from attn_dist and encoder_states
        context_vector = math_ops.reduce_sum(array_ops.reshape(attn_dist, [batch_size, -1, 1, 1]) * encoder_states, [1, 2]) # shape (batch_size, attn_size). # BROADCAST
