...
class SummarizationModel(object):
  ...
  def __init__(self, hps, vocab, extra_info={}):
    self._hps = hps
    self._vocab = vocab
  ...
  def _add_placeholders(self):
    ...
    self._enc_batch_padded = tf.placeholder(tf.int32, [hps.batch_size, hps.max_enc_steps])
  ...
  def _make_feed_dict(self, batch, just_enc=False, extra_input={}):
    ...
    hps=self._hps
    enc_batch_padded = np.ones([hps.batch_size, hps.max_enc_steps], dtype=np.int32) # PAD TOKEN idx = 1
    enc_batch_padded[:,:batch.enc_batch.shape[1]] = batch.enc_batch
    feed_dict[self._enc_batch_padded] = enc_batch_padded
  ...
  def _calc_final_dist(self, vocab_dists, attn_dists):
    ...
    with tf.variable_scope('final_distribution'):
      ...
      # final_dists = [tf.clip_by_value(dist,1e-8,1.0) for dist in final_dists]
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      with tf.variable_scope('embedding'):
        if 'sent2vec' in self._extra_info:
          subnetwork = self._extra_info['sent2vec']['network']
          embedding = subnetwork.embedding_matrix
          print('Use pretrained word embeddings in sent2vec network')
        else:
          embedding = tf.get_variable('embedding', [vsize, hps.emb_dim], dtype=tf.float32, initializer=self.trunc_norm_init)
        assert embedding.get_shape().as_list() == [vsize, hps.emb_dim]
      ...
      final_dists = self._calc_final_dist(vocab_dists, self.attn_dists) if FLAGS.pointer_gen else vocab_dists
      ...
      if hps.mode in ['train', 'eval']:
        ...
        with tf.variable_scope('loss'):
          ...
          if 'sent2vec' in self._extra_info:
            gamma=0.2 if not 'gamma' in self._extra_info else self._extra_info['sent2vec']['gamma']
            subnetwork=self._extra_info['sent2vec']['network']

            loss_mode='dist' if not 'loss_mode' in self._extra_info['sent2vec'] else self._extra_info['sent2vec']['loss_mode']

            # Calculate the embeddings of input and output sequences
            stacked_embeddings=tf.tile(tf.reshape(embedding, [1, vsize, hps.emb_dim]),
              multiples=[hps.batch_size, 1, 1])

            final_dist_over_vocab=[tf.reshape(dist[:,:vsize], [hps.batch_size, 1, vsize]) for dist in final_dists]
            final_dist_over_vocab=tf.concat(final_dist_over_vocab, axis=1)        # tensor with shape [batch_size, max_decode_steps, vsize]

            emb_doc_input=tf.nn.embedding_lookup(embedding, self._enc_batch_padded)      # tensor with shape [batch_size, max_encode_steps, hps.emb_dim]
            emb_sum_input=tf.matmul(final_dist_over_vocab, stacked_embeddings)           # tensor with shape [batch_size, max_decode_steps, hps.emb_dim]

            doc_vec=subnetwork.link_forward(emb_doc_input)         # tensor with shape [batch_size, feature_vector_dim]
            sum_vec=subnetwork.link_forward(emb_sum_input)         # tensor with shape [batch_size, feature_vector_dim]

            if loss_mode.lower() in ['cosine']:
              dot_product=tf.reduce_sum(tf.multiply(doc_vec, sum_vec), axis=1)
              doc_vec_norm=tf.norm(doc_vec, ord=2, axis=1)
              sum_vec_norm=tf.norm(sum_vec, ord=2, axis=1)

              cosine_list=tf.div(dot_product, tf.multiply(doc_vec_norm, sum_vec_norm))
              cosine_loss=tf.reduce_sum(1.0 - cosine_list)

              self._loss=self._loss * (1 - gamma) + cosine_loss * gamma
            elif loss_mode.lower() in ['dist','distance']:
              dist_vec=tf.subtract(doc_vec, sum_vec)
              dist_loss=tf.norm(dist_vec, ord=2, axis=1)          # tensor with shape [batch_size]
              dist_loss=tf.reduce_sum(dist_loss)

              self._loss=self._loss * (1 - gamma) + dist_loss * gamma
            else:
  ...
  def _add_train_op(self):
    ...
    update_pairs=zip(grads, tvars)
    if 'fixed_vars' in self._extra_info:
      fixed_vars=self._extra_info['fixed_vars']
      trainable_update_pairs=[]
      for grad, var in update_pairs:
        if not var in fixed_vars:
          trainable_update_pairs.append((grad,var))
    else:
      trainable_update_pairs=update_pairs
    print('%d variables in total, %d are trainable'%(len(update_pairs), len(trainable_update_pairs)))
    ...
    with tf.device("/cpu:0"):
      self._train_op = optimizer.apply_gradients(trainable_update_pairs, global_step=self.global_step, name='train_step')
